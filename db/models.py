...
class Ticket(models.Model):
    ...
    def __str__(self) -> str:
        formatted_date_time = self.movie_session.show_time.strftime(
            "%Y-%m-%d %H:%M:%S")
        return (f"{self.movie_session.movie.title} {formatted_date_time} "
                f"(row: {self.row}, seat: {self.seat})")

    def clean(self) -> None:
        if not 1 <= self.row <= self.movie_session.cinema_hall.rows:
            error_message = (f"row number must be in available range: "
                             f"(1, rows): "
                             f"(1, {self.movie_session.cinema_hall.rows})")
            raise ValidationError({"row": [error_message]})

        if not 1 <= self.seat <= self.movie_session.cinema_hall.seats_in_row:
            error_message = (f"seat number must be in available range: "
                             f"(1, seats_in_row): "
                             f"(1, "
